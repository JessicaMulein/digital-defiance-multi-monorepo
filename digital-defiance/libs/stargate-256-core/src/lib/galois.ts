// these tables seem to match: https://gitlab.umich.edu/flowerysong/vault/-/blob/7c2c30e5ae4bd286aa55433f607af30ab456efe3/shamir/tables.go
// Tables taken from http://www.samiam.org/galois.html
// They use 0xe5 (229) as the generator

import { IGalois } from "./interfaces.d";

// Some of this class seems to not be used such as seed, generators, and the AES sbox table.
/* TODO: see if we can use a supported NPM package for the Galois field arithmetic instead of this class now that we know
 * where the tables came from. */

export default class Galois implements IGalois {
  /* Exponentiation
   * https://blog.hackeriet.no/AES-math-fundamentals/
   * Generally easy, since exponentiation is just repeated multiplication,
   * but some of the numbers in AES have interesting properties. When you
   * multiply those numbers with themselves they traverse all 255 non-zero
   * numbers in the field. They are called generators and here is a list of
   * them:
   */
  // private static readonly generators: Array<number> = [
  //     0x03, 0x05, 0x06, 0x09, 0x0b, 0x0e, 0x11, 0x12, 0x13, 0x14, 0x17, 0x18,
  //     0x19, 0x1a, 0x1c, 0x1e, 0x1f, 0x21, 0x22, 0x23, 0x27, 0x28, 0x2a, 0x2c,
  //     0x30, 0x31, 0x3c, 0x3e, 0x3f, 0x41, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4b,
  //     0x4c, 0x4e, 0x4f, 0x52, 0x54, 0x56, 0x57, 0x58, 0x59, 0x5a, 0x5b, 0x5f,
  //     0x64, 0x65, 0x68, 0x69, 0x6d, 0x6e, 0x70, 0x71, 0x76, 0x77, 0x79, 0x7a,
  //     0x7b, 0x7e, 0x81, 0x84, 0x86, 0x87, 0x88, 0x8a, 0x8e, 0x8f, 0x90, 0x93,
  //     0x95, 0x96, 0x98, 0x99, 0x9b, 0x9d, 0xa0, 0xa4, 0xa5, 0xa6, 0xa7, 0xa9,
  //     0xaa, 0xac, 0xad, 0xb2, 0xb4, 0xb7, 0xb8, 0xb9, 0xba, 0xbe, 0xbf, 0xc0,
  //     0xc1, 0xc4, 0xc8, 0xc9, 0xce, 0xcf, 0xd0, 0xd6, 0xd7, 0xda, 0xdc, 0xdd,
  //     0xde, 0xe2, 0xe3, 0xe5, 0xe6, 0xe7, 0xe9, 0xea, 0xeb, 0xee, 0xf0, 0xf1,
  //     0xf4, 0xf5, 0xf6, 0xf8, 0xfb, 0xfd, 0xfe, 0xff,
  // ];
  // logTable provides the log(X)/log(g) at each index X
  private static readonly ltable: Uint8Array = new Uint8Array([
    ,
  ]);

  // expTable provides the anti-log or exponentiation value
  // for the equivalent index
  private static readonly etable: Uint8Array = new Uint8Array([
    ,
  ]);

  private readonly gmulInv: Uint8Array;
  public gmulInverse(a: number): number {
    if (a == 0) return 0;
    else return Galois.etable[255 - Galois.ltable[a]];
  }
  public sbox(a: number, key: number): number {
    let c: number, s: number, x: number;
    s = x = this.gmulInverse(a);
    for (c = 0; c < 4; c++) {
      /* One bit circular rotate to the left */
      s = (s << 1) | (s >> 7);
      /* xor with x */
      x ^= s;
    }
    x ^= key; //99; /* 0x63 */
    // cout << x;
    return x;
  }
  public gadd(a: number, b: number): number {
    return a ^ b;
  }
  public gsub(a: number, b: number): number {
    return a ^ b;
  }
  public gmul(a: number, b: number): number {
    let p = 0;
    let counter = 0;
    let hi_bit_set: number;
    for (counter = 0; counter < 8; counter++) {
      if ((b & 1) == 1) p ^= a;
      hi_bit_set = a & 0x80;
      a <<= 1;
      if (hi_bit_set == 0x80) a ^= 0x1b;
      b >>= 1;
    }
    return p;
  }
  private static test() {
    // comparing the math from:
    // https://dev.to/maxart2501/let-s-develop-a-qr-code-generator-part-iii-error-correction-1kbm
    // --------------------------------------------------------------------------------
    const LOG = new Uint8Array(256);
    const EXP = new Uint8Array(256);
    for (let exponent = 1, value = 1; exponent < 256; exponent++) {
      value = value > 127 ? (value << 1) ^ 285 : value << 1;
      LOG[value] = exponent % 255;
      EXP[exponent % 255] = value;
    }
    const mul = function (a: number, b: number) {
      return a && b ? EXP[(LOG[a] + LOG[b]) % 255] : 0;
    };
    const div = function (a: number, b: number) {
      return EXP[(LOG[a] + LOG[b] * 254) % 255];
    };
    const polyMul = function (poly1: Uint8Array, poly2: Uint8Array) {
      // This is going to be the product polynomial, that we pre-allocate.
      // We know it's going to be `poly1.length + poly2.length - 1` long.
      const coeffs = new Uint8Array(poly1.length + poly2.length - 1);

      // Instead of executing all the steps in the example, we can jump to
      // computing the coefficients of the result
      for (let index = 0; index < coeffs.length; index++) {
        let coeff = 0;
        for (let p1index = 0; p1index <= index; p1index++) {
          const p2index = index - p1index;
          // We *should* do better here, as `p1index` and `p2index` could
          // be out of range, but `mul` defined above will handle that case.
          // Just beware of that when implementing in other languages.
          coeff ^= mul(poly1[p1index], poly2[p2index]);
        }
        coeffs[index] = coeff;
      }
      return coeffs;
    };
    // --------------------------------------------------------------------------------
  }
  public gmulLookup(a: number, b: number): number {
    let s: number;
    let q: number;
    const z = 0; // why is this never reassigned?
    s = Galois.ltable[a] + Galois.ltable[b];
    s %= 255;
    s = Galois.etable[s];
    q = s;
    if (a == 0) {
      s = z;
    } else {
      s = q;
    }
    if (b == 0) {
      s = z;
    } else {
      q = z;
    }
    return s;
  }
  public gdiv(a: number, b: number): number {
    const c: number = Galois.ltable[a] - Galois.ltable[b];
    return c % 255;
  }
  public generateGmulInverse(): void {
    this.gmulInv[0] = 0;
    for (let c = 1; c < 256; c++) {
      this.gmulInv[c] = this.gmulInverse(c);
    }
  }
  constructor() {
    this.gmulInv = new Uint8Array(256);
    this.generateGmulInverse();
  }
}

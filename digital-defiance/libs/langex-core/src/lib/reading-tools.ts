// READING TOOLS
import { SettingsReadingTool } from './settingsReadingTool';
...
const updateMaskPosition = (clientY: number, maskHeight: number): void => {
  ...
  setTimeout(() => {
    maskElemBefore?.style.setProperty(
      'height',
      `${Math.max(maskPosition - maskHeight, 0)}px`
    );
...
const updateMaskSettings = (
  clientY: number,
  maskSettings: SettingsReadingTool
): void => {
  if (maskSettings) {
